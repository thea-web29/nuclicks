...
class FacultyController extends Controller
{
    ...
    public function enrollStudent(Request $request)
    {
        ...
        // Send course code email to the enrolled student
        Mail::to($student->email)->send(new CourseCodeMail($course->join_code, $course->name, Auth::user()->name));

        return redirect()->back()->with('success', 'Student enrolled successfully and course code sent to their email.');
    }
...
{
    $course = Course::with(['students', 'materials', 'quizzes.questions'])
    ...
    return view('faculty.course-details', compact('course', 'eligibleStudents'));
}

public function addEligibleStudentsToClass($id)
{
    ...
    // Email all students enrolled in this course's program
    $students = User::where('role', 'student')
    ...
    return redirect()->back()->with('success', "{$added} student(s) added to this class.");
}
    ...
    public function questionBank()
    {
        ...
        return view('faculty.question-bank', compact('questions', 'quizzes'));
    }
    ...
    public function downloadResults($courseId)
    {
        ...
        foreach ($course->students as $student) {
            foreach ($course->quizzes as $quiz) {
                ...
                $percentage = ($attempt && $quiz->total_points > 0) ? round(($attempt->score / $quiz->total_points) * 100, 2) : 0;
                $csvData[] = [
                    $student->name,
                    ...
                    $course->name,
                    $quiz->title,
                    $attempt ? ($attempt->score . '/' . $quiz->total_points) : 'Not attempted',
                    $attempt ? ($percentage . '%') : 'N/A',
                    ($attempt && $attempt->completed_at) ? $attempt->completed_at->format('Y-m-d H:i') : 'N/A',
                    $attempt ? ($attempt->feedback ?? 'N/A') : 'N/A'
                ];
            }
        }
    ...
    /**
     * Generate (or regenerate) the join code for a subject and email it
     * to ALL students whose program + section matches this subject.
     * This replaces the old regenerateCourseCode method.
     */
    public function generateCourseCode($id)
{
    $course = Course::findOrFail($id);
    ...
    foreach ($students as $student) {
        Mail::to($student->email)->send(
            new CourseCodeMail($course->join_code, $course->name, Auth::user()->name)
        );
    ...
    return redirect()->back()->with(
        'success',
        "Join code {$course->join_code} generated and sent to {$sent} student(s)."
    );
}
    ...
    public function createCourse()
    {
        ...
        return view('faculty.create-course', compact('programs'));
    }
    ...
    public function storeCourse(Request $request)
    {
        ...
        $validated['join_code']  = strtoupper(\Illuminate\Support\Str::random(6));

        $course = Course::create($validated);

        return redirect()->route('faculty.course.details', $course->id)
    ...
    public function sendMessage(Request $request)
    {
        ...
        Notification::create([
            'user_id'  => $student->id,
            'type'     => 'message',
            'title'    => 'Message from ' . Auth::user()->name . ': ' . $request->subject,
