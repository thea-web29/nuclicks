...
<head>
    ...
    <title>Activity Logs - Admin Panel | NU Clicks LMS</title>
...
<div class="main-content" id="mainContent">
    <div class="top-bar">
        ...
        <div>
            <h2 class="page-title text-lg md:text-xl">Activity Logs</h2>
            <p class="text-sm text-gray-500 hidden md:block">Track all system activities and user actions</p>
        </div>
        <div class="w-8"></div>
    </div>
            ...
            <div class="stat-card">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-bold" style="color: var(--blue-deep);">{{ $logs->total() }}</p>
                    </div>
                    <i class="ri-history-line text-3xl text-indigo-300"></i>
                </div>
            </div>
            <div class="stat-card">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-gray-500 text-sm">Create Actions</p>
                        <p class="text-3xl font-bold text-green-600">{{ $logs->where('action', 'create')->count() }}</p>
                    </div>
                    <i class="ri-add-line text-3xl text-green-300"></i>
                </div>
            </div>
            <div class="stat-card">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-gray-500 text-sm">Update Actions</p>
                        <p class="text-3xl font-bold text-yellow-600">{{ $logs->where('action', 'update')->count() }}</p>
                    </div>
                    <i class="ri-edit-line text-3xl text-yellow-300"></i>
                </div>
            </div>
            <div class="stat-card">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-gray-500 text-sm">Delete Actions</p>
                        <p class="text-3xl font-bold text-red-600">{{ $logs->where('action', 'delete')->count() }}</p>
                    </div>
                    <i class="ri-delete-bin-line text-3xl text-red-300"></i>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-md p-5 mb-6 border border-gray-100">
            <div class="flex flex-wrap gap-4 items-end">
                <div class="flex-1 min-w-[200px]">
                    ...
                    <input type="text" id="searchLogs" placeholder="Search by user, description..." 
                           class="w-full border border-gray-300 rounded-xl px-4 py-2.5 focus:outline-none focus:border-gold transition">
                </div>
                <div>
                    <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">Action Filter</label>
                    <select id="filterAction" class="px-4 py-2.5 border border-gray-300 rounded-xl focus:outline-none focus:border-gold transition bg-white">
                ...
                <div class="flex gap-2">
                    <button onclick="filterLogs()" class="px-6 py-2.5 rounded-xl text-sm font-semibold transition shadow-sm" style="background: var(--blue-deep); color: white;">
                        <i class="ri-filter-line mr-1"></i> Filter
                    </button>
                    <button onclick="resetFilters()" class="px-6 py-2.5 border border-gray-300 rounded-xl text-sm font-semibold hover:bg-gray-50 transition">
                        Reset
                    </button>
                </div>
            </div>
        ...
        <div class="bg-white rounded-xl shadow-md overflow-hidden border border-gray-100">
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-100" id="logsTableBody">
                        @forelse($logs as $log)
                            <tr class="hover:bg-gray-50 transition log-row">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    <span class="status-badge {{ $color }}">
                                        <i class="{{ $actionIcon }} mr-1 text-xs"></i>
                                        {{ ucfirst($log->action) }}
                                    </span>
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    <div class="flex flex-col">
                                        <span>{{ \Carbon\Carbon::parse($log->created_at)->format('M d, Y') }}</span>
                                        <span class="text-xs text-gray-400">{{ \Carbon\Carbon::parse($log->created_at)->format('h:i A') }}</span>
                                    </div>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="5" class="px-6 py-12 text-center text-gray-500">
                                    <i class="ri-history-line text-6xl text-gray-300 mb-4 block"></i>
...
<script>
    ...
    function filterLogs() {
        ...
        rows.forEach(row => {
            ...
            const action = actionCell ? actionCell.textContent.toLowerCase().trim() : '';
