...
<head>
    ...
    <title>Folder & Files - Admin Portal | NU Horizon LMS</title>
...
<aside class="sidebar" id="sidebar">
    ...
    <div class="sidebar-footer">
        <a href="{{ route('admin.profile') }}" style="display:flex;align-items:center;gap:.75rem;margin-bottom:1rem;text-decoration:none;">
            ...
            <div>
                <p style="color:white;font-weight:600;font-size:.85rem;margin:0;">{{ Auth::user()->name ?? 'Admin User' }}</p>
                <span style="color:rgba(255,255,255,.6);font-size:.7rem;display:block;">{{ Auth::user()->email ?? 'admin@nuclicks.edu' }}</span>
            </div>
        </a>
    ...
    <div class="top-bar">
        <div class="flex items-center gap-3">
            ...
            <div>
                ...
                <p class="text-sm text-gray-500 hidden md:block">Manage repositories, document templates, and faculty materials.</p>
            </div>
        </div>
    ...
    <div class="p-4 md:p-6 space-y-6">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div class="card lg:col-span-1">
                ...
                <form action="{{ route('admin.folder-files.folders.store') }}" method="POST" class="p-5 space-y-4">
                    @csrf
                    <div>
                        <label class="block text-sm font-bold text-gray-700 mb-2">Folder Name</label>
                        <input type="text" name="name" class="form-input" placeholder="e.g., Syllabus Templates" required>
                    </div>
            ...
            <div class="card lg:col-span-2">
                <div class="card-header">
                    ...
                    <span class="text-xs text-gray-500">Repository shared with the Faculty files module</span>
                </div>
                <form action="{{ route('admin.folder-files.upload') }}" method="POST" enctype="multipart/form-data" class="p-5 grid grid-cols-1 md:grid-cols-2 gap-4">
                    @csrf
                    <div>
        ...
        <div class="card">
            <div class="card-header">
                <div>
                    <h3 class="font-bold text-gray-800 flex items-center gap-2">
                        <i class="ri-folder-3-line" style="color: var(--gold);"></i> Repositories
                    </h3>
                    <p class="text-xs text-gray-500 mt-1">Folders created by admins and faculty are shown here.</p>
                </div>
                <a href="{{ route('admin.folder-files') }}" class="btn-light">
                    <i class="ri-home-4-line"></i> Main Repository
                </a>
            </div>

            <div class="p-5 flex flex-wrap gap-3">
                <a href="{{ route('admin.folder-files') }}" class="folder-chip {{ empty($folder) ? 'active' : '' }}">
                    <i class="ri-home-4-line"></i> All Files
                </a>
                @foreach($folders as $item)
                    <a href="{{ route('admin.folder-files', ['folder' => $item->id]) }}" class="folder-chip {{ (string) $folder === (string) $item->id ? 'active' : '' }}">
        ...
        <div class="card">
            <div class="card-header">
                <div>
                    ...
                    <p class="text-xs text-gray-500 mt-1">Admin uploads and active templates are listed together.</p>
                </div>
            </div>
                ...
                <table>
                    ...
                    <tbody>
                        @forelse($files as $file)
                            @php
                                $uploaderRole = strtolower(optional($file->uploader)->role ?? 'admin');
                            @endphp
                            <tr>
                                <td>
                                ...
                                <td>
                                    <span class="badge {{ $uploaderRole === 'admin' ? 'badge-admin' : 'badge-faculty' }}">
                                        ...
                                        {{ optional($file->uploader)->name ?? 'Admin' }}
                                    </span>
                                </td>
                                <td>{{ $file->size_for_humans }}</td>
                                <td>
                                    <div class="flex flex-wrap gap-2">
                                        <a href="{{ route('admin.folder-files.download', $file) }}" class="btn-light">
                                            <i class="ri-download-line"></i> Download
                                        </a>

                                        <form action="{{ route('admin.folder-files.archive', $file) }}" method="POST" onsubmit="return confirm('Archive this file?');" style="margin:0;">
                                            @csrf
                                            @method('PATCH')
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="text-center py-10 text-gray-500">
                                    ...
                                    No active files found in this folder repository.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
        ...
        <div class="card">
            <div class="card-header">
                <div>
                    <h3 class="font-bold text-gray-800 flex items-center gap-2">
                        <i class="ri-folder-shared-line" style="color: var(--gold);"></i> Faculty Uploaded Files / Materials
                    </h3>
                    <p class="text-xs text-gray-500 mt-1">Files uploaded by academic faculty for courses and subjects.</p>
                </div>
            </div>
                                ...
                                <td>
                                    <div class="flex items-start gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-xs text-gray-500 mt-1 uppercase">{{ $material->file_type ?? 'PDF' }}</p>
                                        </div>
                                    </div>
                                </td>
                                <td>
                                    <span class="badge badge-file">{{ $material->course->name ?? 'N/A' }}</span>
                                </td>
                                <td>
                                    <span class="badge badge-faculty">
                                        <i class="ri-user-star-line"></i>
                                        {{ $material->course->faculty->name ?? 'N/A' }}
                                    </span>
                                </td>
                                <td>
                                    <span class="text-xs text-gray-500 font-bold">{{ $material->created_at->format('M d, Y') }}</span>
                                </td>
                                <td>
                                    @if($material->file_path)
                                        <a href="{{ asset('storage/' . $material->file_path) }}" target="_blank" class="btn-light">
                                            <i class="ri-external-link-line"></i> Open File
                                        </a>
                                    @else
                                        <span class="badge badge-navy">No file</span>
                                    @endif
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="5" class="text-center py-10 text-gray-500">
                                    <i class="ri-draft-line text-5xl text-gray-300 block mb-3"></i>
                                    No faculty materials uploaded yet.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
            ...
            @if($facultyMaterials->hasPages())
                <div class="p-4 border-t border-gray-100">
                    {{ $facultyMaterials->appends(request()->query())->links() }}
                </div>
            @endif
        </div>
        ...
        <div class="card">
            <div class="card-header">
                <div>
                    <h3 class="font-bold text-gray-800 flex items-center gap-2">
                        <i class="ri-archive-line" style="color: var(--gold);"></i> Archived Files
                    </h3>
                    <p class="text-xs text-gray-500 mt-1">Archived documents and repository items.</p>
                </div>
            </div>
            ...
            <div class="p-6">
                @if($archivedFiles->count() > 0)
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        @foreach($archivedFiles as $file)
                            <div class="border border-gray-150 rounded-xl p-4 bg-slate-50/50 hover:bg-slate-50 transition shadow-sm flex items-center gap-3">
                                <div class="w-10 h-10 rounded-xl bg-orange-100 text-orange-600 flex items-center justify-center text-xl shrink-0">
                                    <i class="ri-file-zip-line"></i>
                                </div>
                                <div class="min-w-0">
                                    <p class="font-bold text-gray-800 truncate" style="font-size:0.9rem;">{{ $file->original_name ?? $file->name }}</p>
                                    <p class="text-xs text-gray-500 font-medium">Archived on {{ $file->archived_at?->format('M d, Y') }}</p>
                                </div>
                            </div>
                        @endforeach
                    </div>
                @else
                    <div class="text-center py-6 text-gray-500">
                        <i class="ri-archive-line text-4xl block mb-2 opacity-40"></i>
                        No archived files exist in this repository.
                    </div>
                @endif
            </div>
        </div>
