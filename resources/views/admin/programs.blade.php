<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Program/Course Management - NU Clicks LMS</title>
...
<div class="main-content" id="mainContent"><div class="top-bar"><button class="menu-toggle" id="menuToggle"><i class="ri-menu-line"></i></button><div><h2 style="font-weight:700;color:var(--blue-deep);font-size:1.1rem;">Program/Course Management</h2><p class="text-sm text-gray-500 hidden md:block">Each program/course belongs to one department</p></div><div class="flex items-center gap-3">@if(View::exists('admin.partials.notification-bell')) @include('admin.partials.notification-bell') @endif<button onclick="openCreateModal()" class="flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-semibold" style="background:var(--blue-deep);color:white;"><i class="ri-add-line"></i>Add Program/Course</button></div></div><div class="p-4 md:p-6"><div class="table-card"><div class="overflow-x-auto"><table class="data-table"><thead><tr><th>Code</th><th>Program/Course Name</th><th>Department</th><th>Description</th><th>Sections</th><th>Subjects</th><th>Students</th><th>Actions</th></tr></thead><tbody>@forelse($programs as $program)<tr><td><span class="badge badge-navy">{{ $program->code }}</span></td><td><div class="font-semibold text-gray-900">{{ $program->name }}</div></td><td>@if($program->department)<span class="badge badge-gold">{{ $program->department->code }}</span><div class="text-xs text-gray-500 mt-1">{{ $program->department->name }}</div>@else<span class="text-red-500 text-sm">No department</span>@endif</td><td><div class="text-sm text-gray-500">{{ Str::limit($program->description, 60) ?: '—' }}</div></td><td><span class="badge badge-blue">{{ $program->sections_count ?? 0 }}</span></td><td><span class="badge badge-blue">{{ $program->subjects_count ?? 0 }}</span></td><td><span class="badge badge-green">{{ $program->students_count ?? 0 }}</span></td><td><div class="flex items-center gap-3"><button onclick="editProgram({{ $program->id }})" class="text-yellow-600 btn-icon"><i class="ri-edit-line text-lg"></i></button><button onclick="deleteProgram({{ $program->id }}, '{{ addslashes($program->name) }}')" class="text-red-600 btn-icon"><i class="ri-delete-bin-line text-lg"></i></button></div></td></tr>@empty<tr><td colspan="8" class="px-6 py-12 text-center text-gray-500">No programs/courses yet.</td></tr>@endforelse</tbody></table></div></div></div></div>
<div id="programModal" class="modal-overlay"><div class="modal-box"><div class="modal-header"><h3><i class="ri-graduation-cap-line"></i><span id="modalTitle">Add Program/Course</span></h3><button class="modal-close" onclick="closeModal()">&times;</button></div><div class="p-6"><form id="programForm">@csrf<input type="hidden" id="programId"><div class="mb-4"><label class="block text-sm font-semibold text-gray-700 mb-2">Department *</label><select id="departmentId" required class="w-full border border-gray-300 rounded-xl px-4 py-2.5"><option value="">Select Department</option>@foreach($departments as $department)<option value="{{ $department->id }}">{{ $department->code }} - {{ $department->name }}</option>@endforeach</select></div><div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4"><div><label class="block text-sm font-semibold text-gray-700 mb-2">Code *</label><input type="text" id="programCode" required maxlength="20" class="w-full border border-gray-300 rounded-xl px-4 py-2.5 uppercase" placeholder="BSIT"></div><div><label class="block text-sm font-semibold text-gray-700 mb-2">Program/Course Name *</label><input type="text" id="programName" required class="w-full border border-gray-300 rounded-xl px-4 py-2.5" placeholder="Bachelor of Science in Information Technology"></div></div><div class="mb-6"><label class="block text-sm font-semibold text-gray-700 mb-2">Description</label><textarea id="programDesc" rows="3" class="w-full border border-gray-300 rounded-xl px-4 py-2.5"></textarea></div><div class="flex justify-end gap-3 pt-4 border-t"><button type="button" onclick="closeModal()" class="px-5 py-2.5 border rounded-xl">Cancel</button><button type="submit" id="submitBtn" class="px-6 py-2.5 rounded-xl text-sm font-semibold" style="background:var(--blue-deep);color:white;">Save Program/Course</button></div></form></div></div></div>
<script>
function openCreateModal(){document.getElementById('modalTitle').innerText='Add Program/Course';document.getElementById('programForm').reset();document.getElementById('programId').value='';document.getElementById('submitBtn').innerText='Save Program/Course';document.getElementById('programModal').classList.add('active');document.body.style.overflow='hidden'}function closeModal(){document.getElementById('programModal').classList.remove('active');document.body.style.overflow=''}function editProgram(id){fetch(`/admin/programs/${id}/data`,{headers:{Accept:'application/json','X-Requested-With':'XMLHttpRequest'}}).then(r=>r.json()).then(d=>{if(d.success){document.getElementById('modalTitle').innerText='Edit Program/Course';document.getElementById('programId').value=d.program.id;document.getElementById('departmentId').value=d.program.department_id||'';document.getElementById('programCode').value=d.program.code||'';document.getElementById('programName').value=d.program.name||'';document.getElementById('programDesc').value=d.program.description||'';document.getElementById('submitBtn').innerText='Update Program/Course';document.getElementById('programModal').classList.add('active');document.body.style.overflow='hidden'}else alert(d.message||'Failed to load program')})}function deleteProgram(id,name){if(!confirm(`Delete program/course "${name}"?\n\nPrograms with sections, subjects, or students cannot be deleted.`))return;fetch(`/admin/programs/${id}`,{method:'DELETE',headers:{'X-CSRF-TOKEN':'{{ csrf_token() }}',Accept:'application/json','X-Requested-With':'XMLHttpRequest'}}).then(r=>r.json()).then(d=>{if(d.success)location.reload();else alert(d.message||'Error deleting program/course')})}document.getElementById('programForm').addEventListener('submit',e=>{e.preventDefault();const id=document.getElementById('programId').value,url=id?`/admin/programs/${id}`:'/admin/programs',fd=new FormData();fd.append('_token','{{ csrf_token() }}');if(id)fd.append('_method','PUT');fd.append('department_id',document.getElementById('departmentId').value);fd.append('code',document.getElementById('programCode').value.trim().toUpperCase());fd.append('name',document.getElementById('programName').value.trim());fd.append('description',document.getElementById('programDesc').value.trim());fetch(url,{method:'POST',body:fd,headers:{'X-Requested-With':'XMLHttpRequest'}}).then(r=>r.json()).then(d=>{if(d.success)location.reload();else alert(d.message||(d.errors?Object.values(d.errors).flat().join('\n'):'Error saving program/course'))})});document.getElementById('programCode').addEventListener('input',function(){this.value=this.value.toUpperCase()});document.getElementById('programModal').addEventListener('click',e=>{if(e.target.id==='programModal')closeModal()});
</script>
