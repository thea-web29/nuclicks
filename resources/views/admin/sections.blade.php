...
<div class="main-content" id="mainContent"><div class="top-bar"><button class="menu-toggle" id="menuToggle"><i class="ri-menu-line"></i></button><div><h2 style="font-weight:700;color:var(--blue-deep);font-size:1.1rem;">Section Management</h2><p class="text-sm text-gray-500 hidden md:block">Sections must be unique under each program/course</p></div><div class="flex items-center gap-3">@if(View::exists('admin.partials.notification-bell')) @include('admin.partials.notification-bell') @endif<button onclick="openCreateModal()" class="flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-semibold" style="background:var(--blue-deep);color:white;"><i class="ri-add-line"></i>Add Section</button></div></div><div class="p-4 md:p-6"><div class="table-card"><div class="overflow-x-auto"><table class="data-table"><thead><tr><th>Section</th><th>Program/Course</th><th>Department</th><th>Year Level</th><th>Academic Year</th><th>Subjects</th><th>Faculty Assignments</th><th>Actions</th></tr></thead><tbody>@forelse($sections as $section)<tr><td><span class="badge badge-navy">{{ $section->name }}</span></td><td><div class="font-semibold">{{ $section->program->name ?? '—' }}</div><span class="text-xs text-gray-500">{{ $section->program->code ?? '' }}</span></td><td>@if($section->program && $section->program->department)<span class="badge badge-gold">{{ $section->program->department->code }}</span>@else<span class="text-red-500 text-sm">No department</span>@endif</td><td>{{ $section->year_level ?: '—' }}</td><td>{{ $section->academic_year ?: '—' }}</td><td><span class="badge badge-blue">{{ $section->subjects_count ?? 0 }}</span></td><td><span class="badge badge-green">{{ $section->faculty_subject_assignments_count ?? 0 }}</span></td><td><div class="flex gap-3"><button onclick="editSection({{ $section->id }})" class="text-yellow-600"><i class="ri-edit-line text-lg"></i></button><button onclick="deleteSection({{ $section->id }}, '{{ addslashes($section->name) }}')" class="text-red-600"><i class="ri-delete-bin-line text-lg"></i></button></div></td></tr>@empty<tr><td colspan="8" class="text-center py-12 text-gray-500">No sections yet.</td></tr>@endforelse</tbody></table></div></div></div></div>
<div id="sectionModal" class="modal-overlay"><div class="modal-box"><div class="modal-header"><h3><i class="ri-layout-grid-line"></i><span id="modalTitle">Add Section</span></h3><button class="modal-close" onclick="closeModal()">&times;</button></div><div class="p-6"><form id="sectionForm">@csrf<input type="hidden" id="sectionId"><div class="mb-4"><label class="block text-sm font-semibold mb-2">Program/Course *</label><select id="programId" required class="w-full border rounded-xl px-4 py-2.5"><option value="">Select Program/Course</option>@foreach($programs as $program)<option value="{{ $program->id }}">{{ $program->code }} - {{ $program->name }} @if($program->department) ({{ $program->department->code }}) @endif</option>@endforeach</select></div><div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4"><div><label class="block text-sm font-semibold mb-2">Section Name *</label><input id="sectionName" required class="w-full border rounded-xl px-4 py-2.5 uppercase" placeholder="1A"></div><div><label class="block text-sm font-semibold mb-2">Year Level</label><input id="yearLevel" class="w-full border rounded-xl px-4 py-2.5" placeholder="1st Year"></div><div><label class="block text-sm font-semibold mb-2">Academic Year</label><input id="academicYear" class="w-full border rounded-xl px-4 py-2.5" placeholder="2026-2027"></div></div><div class="mb-6"><label class="block text-sm font-semibold mb-2">Description</label><textarea id="sectionDesc" rows="3" class="w-full border rounded-xl px-4 py-2.5"></textarea></div><div class="flex justify-end gap-3 pt-4 border-t"><button type="button" onclick="closeModal()" class="px-5 py-2.5 border rounded-xl">Cancel</button><button type="submit" id="submitBtn" class="px-6 py-2.5 rounded-xl font-semibold" style="background:var(--blue-deep);color:white;">Save Section</button></div></form></div></div></div>
<script>
function openCreateModal(){sectionForm.reset();sectionId.value='';modalTitle.innerText='Add Section';submitBtn.innerText='Save Section';sectionModal.classList.add('active');document.body.style.overflow='hidden'}function closeModal(){sectionModal.classList.remove('active');document.body.style.overflow=''}function editSection(id){fetch(`/admin/sections/${id}/data`,{headers:{Accept:'application/json','X-Requested-With':'XMLHttpRequest'}}).then(r=>r.json()).then(d=>{if(d.success){modalTitle.innerText='Edit Section';sectionId.value=d.section.id;programId.value=d.section.program_id||'';sectionName.value=d.section.name||'';yearLevel.value=d.section.year_level||'';academicYear.value=d.section.academic_year||'';sectionDesc.value=d.section.description||'';submitBtn.innerText='Update Section';sectionModal.classList.add('active');document.body.style.overflow='hidden'}else alert(d.message||'Failed to load section')})}function deleteSection(id,name){if(!confirm(`Delete section "${name}"?`))return;fetch(`/admin/sections/${id}`,{method:'DELETE',headers:{'X-CSRF-TOKEN':'{{ csrf_token() }}',Accept:'application/json','X-Requested-With':'XMLHttpRequest'}}).then(r=>r.json()).then(d=>{if(d.success)location.reload();else alert(d.message||'Error deleting section')})}sectionForm.addEventListener('submit',e=>{e.preventDefault();const id=sectionId.value,url=id?`/admin/sections/${id}`:'/admin/sections',fd=new FormData();fd.append('_token','{{ csrf_token() }}');if(id)fd.append('_method','PUT');fd.append('program_id',programId.value);fd.append('name',sectionName.value.trim().toUpperCase());fd.append('year_level',yearLevel.value.trim());fd.append('academic_year',academicYear.value.trim());fd.append('description',sectionDesc.value.trim());fetch(url,{method:'POST',body:fd,headers:{'X-Requested-With':'XMLHttpRequest'}}).then(r=>r.json()).then(d=>{if(d.success)location.reload();else alert(d.message||(d.errors?Object.values(d.errors).flat().join('\n'):'Error saving section'))})});sectionName.addEventListener('input',function(){this.value=this.value.toUpperCase()});sectionModal.addEventListener('click',e=>{if(e.target.id==='sectionModal')closeModal()});
</script>
