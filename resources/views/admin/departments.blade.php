...
<div class="main-content" id="mainContent">
  <div class="top-bar"><button class="menu-toggle" id="menuToggle"><i class="ri-menu-line"></i></button><div><h2 style="font-weight:700;color:var(--blue-deep);font-size:1.1rem;">Department Management</h2><p class="text-sm text-gray-500 hidden md:block">Department → Program/Course → Section → Subject</p></div><div class="flex items-center gap-3">@if(View::exists('admin.partials.notification-bell')) @include('admin.partials.notification-bell') @endif<button onclick="openCreateModal()" class="flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-semibold transition shadow-sm" style="background:var(--blue-deep);color:white;"><i class="ri-add-line"></i>Add Department</button></div></div>
  <div class="p-4 md:p-6">@if(session('success'))<div class="bg-green-100 border-l-4 border-green-500 text-green-800 px-4 py-3 rounded-xl mb-6">{{ session('success') }}</div>@endif
    <div class="table-card"><div class="overflow-x-auto"><table class="data-table"><thead><tr><th>Code</th><th>Department Name</th><th>Programs/Courses</th><th>Description</th><th>Faculty</th><th>Actions</th></tr></thead><tbody>@forelse($departments as $department)<tr><td><span class="badge badge-navy">{{ $department->code ?? '—' }}</span></td><td><div class="font-semibold text-gray-900">{{ $department->name ?? 'Unnamed Department' }}</div></td><td><div class="flex flex-wrap gap-1">@forelse($department->programs as $program)<span class="badge badge-gold">{{ $program->code }}</span>@empty<span class="text-sm text-gray-400">No programs yet</span>@endforelse</div><div class="text-xs text-gray-400 mt-1">{{ $department->programs_count ?? $department->programs->count() }} program/course(s)</div></td><td><div class="text-sm text-gray-500">{{ !empty($department->description) ? Str::limit($department->description, 70) : '—' }}</div></td><td><span class="badge badge-blue">{{ $department->computed_faculty_count ?? 0 }} faculty</span></td><td><div class="flex items-center gap-3"><button onclick="editDepartment({{ $department->id }})" class="text-yellow-600 hover:text-yellow-800 btn-icon" title="Edit"><i class="ri-edit-line text-lg"></i></button><button onclick="deleteDepartment({{ $department->id }}, '{{ addslashes($department->name ?? 'this department') }}')" class="text-red-600 hover:text-red-800 btn-icon" title="Delete"><i class="ri-delete-bin-line text-lg"></i></button></div></td></tr>@empty<tr><td colspan="6" class="px-6 py-12 text-center text-gray-500"><i class="ri-building-2-line text-6xl text-gray-300 mb-4 block"></i>No departments yet.</td></tr>@endforelse</tbody></table></div></div>
  </div>
</div>
<div id="deptModal" class="modal-overlay"><div class="modal-box"><div class="modal-header"><h3><i class="ri-building-2-line"></i><span id="modalTitle">Add Department</span></h3><button class="modal-close" onclick="closeModal()">&times;</button></div><div class="p-6"><form id="deptForm">@csrf<input type="hidden" id="deptId"><div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4"><div><label class="block text-sm font-semibold text-gray-700 mb-2">Department Code *</label><input type="text" id="deptCode" required maxlength="20" class="w-full border border-gray-300 rounded-xl px-4 py-2.5 uppercase" placeholder="SACE"></div><div><label class="block text-sm font-semibold text-gray-700 mb-2">Department Name *</label><input type="text" id="deptName" required class="w-full border border-gray-300 rounded-xl px-4 py-2.5" placeholder="School of Architecture, Computing & Engineering"></div></div><div class="mb-6"><label class="block text-sm font-semibold text-gray-700 mb-2">Description</label><textarea id="deptDesc" rows="3" class="w-full border border-gray-300 rounded-xl px-4 py-2.5"></textarea></div><div class="flex justify-end gap-3 pt-4 border-t border-gray-100"><button type="button" onclick="closeModal()" class="px-5 py-2.5 border border-gray-300 rounded-xl">Cancel</button><button type="submit" id="submitBtn" class="px-6 py-2.5 rounded-xl text-sm font-semibold" style="background:var(--blue-deep);color:white;">Save Department</button></div></form></div></div></div>
<script>
function openCreateModal(){document.getElementById('modalTitle').innerText='Add Department';document.getElementById('deptForm').reset();document.getElementById('deptId').value='';document.getElementById('submitBtn').innerText='Save Department';document.getElementById('deptModal').classList.add('active');document.body.style.overflow='hidden'}function closeModal(){document.getElementById('deptModal').classList.remove('active');document.body.style.overflow=''}function editDepartment(id){fetch(`/admin/departments/${id}/data`,{headers:{Accept:'application/json','X-Requested-With':'XMLHttpRequest'}}).then(r=>r.json()).then(d=>{if(d.success){document.getElementById('modalTitle').innerText='Edit Department';document.getElementById('deptId').value=d.department.id;document.getElementById('deptCode').value=d.department.code||'';document.getElementById('deptName').value=d.department.name||'';document.getElementById('deptDesc').value=d.department.description||'';document.getElementById('submitBtn').innerText='Update Department';document.getElementById('deptModal').classList.add('active');document.body.style.overflow='hidden'}else alert(d.message||'Failed to load department')})}function deleteDepartment(id,name){if(!confirm(`Delete department "${name}"?\n\nDepartments with programs/courses cannot be deleted.`))return;fetch(`/admin/departments/${id}`,{method:'DELETE',headers:{'X-CSRF-TOKEN':'{{ csrf_token() }}',Accept:'application/json','X-Requested-With':'XMLHttpRequest'}}).then(r=>r.json()).then(d=>{if(d.success)location.reload();else alert(d.message||'Error deleting department')})}document.getElementById('deptForm').addEventListener('submit',e=>{e.preventDefault();const id=document.getElementById('deptId').value,url=id?`/admin/departments/${id}`:'/admin/departments',fd=new FormData();fd.append('_token','{{ csrf_token() }}');if(id)fd.append('_method','PUT');fd.append('code',document.getElementById('deptCode').value.trim().toUpperCase());fd.append('name',document.getElementById('deptName').value.trim());fd.append('description',document.getElementById('deptDesc').value.trim());fetch(url,{method:'POST',body:fd,headers:{'X-Requested-With':'XMLHttpRequest'}}).then(r=>r.json()).then(d=>{if(d.success)location.reload();else alert(d.message||(d.errors?Object.values(d.errors).flat().join('\n'):'Error saving department'))})});document.getElementById('deptCode').addEventListener('input',function(){this.value=this.value.toUpperCase()});document.getElementById('deptModal').addEventListener('click',e=>{if(e.target.id==='deptModal')closeModal()});
</script>
