...
<head>
    ...
    <title>Announcements – NU Horizon LMS</title>
    ...
    <style>
        ...
        *:focus { outline: none !important; }
        ...
        .announcement-item:hover { background: var(--gold-pale); }

        .announcement-top { display: flex; align-items: flex-start; justify-content: space-between; gap: 1rem; margin-bottom: .75rem; }
        .announcement-meta { display: flex; align-items: center; gap: .5rem; flex-wrap: wrap; margin-bottom: .45rem; }
        ...
        .course-name-label { font-size: .72rem; color: var(--txt-3); }
        .announcement-title { font-family: 'Fraunces', serif; font-size: 1rem; font-weight: 700; color: var(--navy); line-height: 1.3; margin-bottom: .4rem; }
        .announcement-content { font-size: .8rem; color: var(--txt-2); line-height: 1.6; }
        .announcement-date { text-align: right; flex-shrink: 0; }
        .date-day { font-size: .78rem; font-weight: 700; color: var(--txt-2); }
        .date-time { font-size: .68rem; color: var(--txt-3); margin-top: .1rem; }
        .announcement-footer { display: flex; align-items: center; gap: 1.2rem; margin-top: .75rem; }
        .ann-meta-item { font-size: .7rem; color: var(--txt-3); display: flex; align-items: center; gap: .3rem; }
        .ann-meta-item i { font-size: .75rem; }
        ...
        .empty-card i { font-size: 3rem; display: block; margin-bottom: .75rem; color: var(--txt-3); opacity: .3; }
        ...
        @media (max-width: 640px) {
            ...
            .announcement-top { flex-direction: column; }
            .announcement-date { text-align: left; }
        }
    </style>
</head>
<body>
...
<div class="main-content" id="mainContent">
    <header class="topbar">
        <div class="topbar-left">
            ...
            <div>
                ...
                <div class="topbar-breadcrumb">Student → Announcements</div>
            </div>
        </div>
    ...
    <div class="page-body">
        ...
        <div class="section-header">
            <h2><i class="ri-megaphone-line" style="color:var(--gold-d);font-size:.85rem;"></i> Announcements</h2>
        </div>
            ...
            <div class="dash-card">
                <div class="dash-card-head">
                    <div class="dash-card-title">
                        <i class="ri-megaphone-line"></i> All Announcements
                    </div>
                    <span class="dash-card-badge">{{ $announcements->total() }} total</span>
                </div>

                @foreach($announcements as $i => $announcement)
                    <div class="announcement-item" style="animation-delay:{{ $i * 0.04 }}s;">
                        <div class="announcement-top">
                            <div style="flex:1;min-width:0;">
                                <div class="announcement-meta">
                                    <span class="course-code-pill">{{ $announcement->course->code }}</span>
                                    <span class="course-name-label">{{ $announcement->course->name }}</span>
                                </div>
                                <div class="announcement-title">{{ $announcement->title }}</div>
                                <div class="announcement-content">{{ $announcement->content }}</div>
                            </div>
                            <div class="announcement-date">
                                <div class="date-day">{{ $announcement->created_at->format('M d, Y') }}</div>
                                <div class="date-time">{{ $announcement->created_at->format('h:i A') }}</div>
                            </div>
                        </div>
                        <div class="announcement-footer">
                            <span class="ann-meta-item">
                                <i class="ri-user-line"></i> {{ $announcement->faculty->name ?? 'Instructor' }}
                            </span>
                            <span class="ann-meta-item">
                                <i class="ri-time-line"></i> {{ $announcement->created_at->diffForHumans() }}
                            </span>
                        </div>
                    </div>
                @endforeach
        ...
        @else
            <div class="empty-card">
                ...
                <p>Your instructors haven't posted any announcements yet. Check back later for important updates.</p>
            </div>
        @endif
